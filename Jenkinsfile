pipeline {
    ...
    stages {
        ...
        /* 🔥 FIX: NO DEMO DATA */
        stage('Init Odoo 17 (NO DEMO DATA)') {
            steps {
                sh '''
        ...
        /* ================= OPENUPGRADE CLEANUPS ================= */

        stage('Disable Demo Flags Explicitly') {
            steps {
                sh '''
...
SQL
        ...
        stage('OpenUpgrade - Clean Base Views & Languages') {
            steps {
                sh '''
...
SQL
        ...
        stage('Post Checks') {
            steps {
                sh '''
                  docker ps
                  echo "✅ Login → Settings → About → Version = Odoo 18"
                  df -h
                '''
            }
        }
